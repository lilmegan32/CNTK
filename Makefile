...
java: $(JAVA_LIBS)
	...
	$(CXX) -shared $(COMMON_FLAGS) $(CPPFLAGS) $(CXXFLAGS) $(INCLUDEPATH:%=-I%) $(JDK_INCLUDE_PATH:%=-I%) -DSWIG -DSWIGJAVA $(JAVA_SWIG_DIR)/cntk_java_wrap.cxx -L$(LIBDIR) -l$(CNTKMATH) -l$(CNTKLIBRARY) -L$(PROTOBUF_PATH)/lib -lprotobuf -o $(LIBDIR)/libCntk.Core.JavaBinding-$(CNTK_COMPONENT_VERSION).so
