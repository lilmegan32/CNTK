...
java: $(JAVA_LIBS)
	...
	$(CXX) $(LDFLAGS) -shared $(COMMON_FLAGS) $(CPPFLAGS) $(CXXFLAGS) $(INCLUDEPATH:%=-I%) $(JDK_INCLUDE_PATH:%=-I%) -L$(LIBDIR) -l$(CNTKMATH) -l$(CNTKLIBRARY) -L$(PROTOBUF_PATH)/lib -lprotobuf $(patsubst %,$(RPATH)%, $(ORIGINDIR)) -o $(LIBDIR)/libCntk.Core.JavaBinding-$(CNTK_COMPONENT_VERSION).so $(JAVA_SWIG_DIR)/cntk_java_wrap.cxx
